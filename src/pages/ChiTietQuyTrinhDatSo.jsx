...
export default function ChiTietQuyTrinhDatSo() {
  ...
  async function getQuyTrinh(id) {
    ...
    if (data) {
      setQuyTrinh({
        ...data,
        NgaySuDung:data.NgaySuDungUnix!==0?new Date(data.NgaySuDungUnix*1000):null,
        GhiChu: data.GhiChu || "",
      });
    }
  }
  ...
  async function getAllOptions() {
    ...
    // console.log((quyTrinh.idNguoiDat === null || quyTrinh.idNguoiDat === undefined));
    ...
    // }
    if(opt === "add" && res[3] !== null && res[3] !== undefined && res[3].idNhanSu > 0)
    {
      setForm(res[3].idNhanSu, "idNguoiDat");
    }
  }
  ...
  const handleApprove = async() => {
    if (validate()) {
      let res = await QuyTrinhService.DatSoCongVanDi.ChuyenTiep(quyTrinh);
      if (res && res.Error === 4) {
        console.log(res);
        toast.success(res.Detail);
        navigate(-1);
  ...
  const handleReject = async() => {
    if (validate()) {
      let res = await QuyTrinhService.DatSoCongVanDi.KhongDuyet(quyTrinh);
      if (res && res.Error === 4) {
        console.log(res);
        toast.success(res.Detail);
        navigate(-1);
  ...
  const handleDelete = async() => {
    // if (validate()) {
      let res = await QuyTrinhService.DatSoCongVanDi.Delete(quyTrinh.IdQuyTrinh);
      if (res && res.Error === 4) {
        console.log(res);
        toast.success(res.Detail);
        navigate(-1);
