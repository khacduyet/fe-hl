...
export default function DanhMucLoaiDongPhi() {
  ...
  const getList = async () => {
    let list = await DanhMucService.LoaiDongPhi.GetList();
    if (list) {
  ...
  const confirmAdd = async () => {
    if (validate()) {
      let add = await DanhMucService.LoaiDongPhi.Set(item);
      if (add) {
  ...
  const handleDelete = (item) => {
    confirmDialog({
      ...
      rejectLabel: "Hủy",
      accept: async () => {
        let $delete = await DanhMucService.LoaiDongPhi.Delete(item.Id);
        if ($delete) {
  ...
  return (
    <>
      <h1 className="section-heading">Danh mục loại đóng phí</h1>
