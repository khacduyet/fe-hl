...
export default function DanhMucLoaiTietHoc() {
  ...
  const getListDanhMuc = async () => {
    ...
    if (list) {
      ...
      list.forEach((item, index) => {
        item.STT = index + 1;
        item.TenCaHoc = listCaHoc.find((CaHoc) => CaHoc.Id === item.IdCaHoc)?.Ten || null;
      });
      setListItem(list);
    }
  };
  ...
  const confirmAdd = async () => {
    if (validate()) {
      let add = await DanhMucService.LoaiTietHoc[item.Id === "" ? "Add" : "Update"](
        item
      );
      if (add) {
        console.log(add);
  ...
  const handleDelete = (item) => {
    confirmDialog({
      ...
      rejectLabel: "Hủy",
      accept: async () => {
        let $delete = await DanhMucService.LoaiTietHoc.Delete(item);
        if ($delete) {
          if ($delete.Error === 4) {
  ...
  return (
    <>
      <h1 className="section-heading">Danh mục loại tiết học</h1>
      <div className="container-haha">
        ...
        <DataTable className="p-datatable-sm p-datatable-gridlines pt-5" value={listItem} paginatorLeft={'Tổng số bản ghi ' + listItem?.length} paginatorClassName="justify-content-end" paginator first={0} rows={10}>
          ...
          <Column
            style={{ width: '20%' }} 
            field="Ten"
            headerClassName="text-center" bodyClassName="text-center"
            header="Tên"
          ></Column>
          <Column
            style={{ width: '15%' }} 
          ...
          <div className="formgrid grid pt-3">
            ...
            <div className="field col-12">
              ...
              <InputNumber
                value={item.SoPhut}
                className="w-full"
                onValueChange={(e) => setForm(e.value, "SoPhut")}
                mode="decimal"
                minFractionDigits={0}
              />
            </div>
