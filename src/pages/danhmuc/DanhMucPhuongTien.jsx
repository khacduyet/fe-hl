...
export default function DanhMucPhuongTien() {
  ...
  const getList = async () => {
    let list = await DanhMucService.PhuongTien.GetList();
    if (list) {
  ...
  const confirmAdd = async () => {
    if (validate()) {
      let add = await DanhMucService.PhuongTien.Set(item);
      if (add) {
  ...
  const handleDelete = (item) => {
    confirmDialog({
      ...
      rejectLabel: "Hủy",
      accept: async () => {
        let $delete = await DanhMucService.PhuongTien.Delete(item.Id);
        if ($delete) {
  ...
  return (
    <>
      <h1 className="section-heading">Danh mục phương tiện</h1>
